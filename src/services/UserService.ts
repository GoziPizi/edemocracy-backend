...
class UserService {
  ...
  static async getUserOpinions(id: string) {
    return await UserService.opinionRepository.findOpinionsWithTitleByUserId(
      id
    );
  }
  ...
  static async follow(id: string, entityId: string, entityType: string) {
    const actualFollow =
      await UserService.userRepository.findSingleFollowByUserId(id, entityId);
    if (actualFollow) {
  ...
  static async isUserFollowing(id: string, entityId: string) {
    const follow = await UserService.userRepository.findSingleFollowByUserId(
      id,
      entityId
    );
    if (follow) {
  ...
  static async postOpinion(userId: string, topicId: string, opinion: string) {
    return await UserService.opinionRepository.createOpinion(
      userId,
      topicId,
      opinion
    );
  ...
  static async getUserDiplomas(id: string) {
    return await UserService.userRepository.getUserDiplomas(id);
  }

  static async updateUserDiplomas(
    id: string,
    diplomas: { name: string; obtention: number }[]
  ) {
    return await UserService.userRepository.updateUserDiplomas(id, diplomas);
  }
}
