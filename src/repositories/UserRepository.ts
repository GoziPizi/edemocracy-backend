...
class UserRepository extends PrismaRepository {
  ...
  create = async (user: any) => {
    const hashedPassword = await this.RawQueryRepository.getSha256(
      user.password
    );
  ...
  getUserDiplomas = async (userId: string) => {
    const diplomas = await this.prismaClient.userDiploma.findMany({
      where: {
        userId,
      },
    });
  ...
  updateUserDiplomas = async (
    userId: string,
    diplomas: { name: string; obtention: number }[]
  ) => {
    try {
      await this.prismaClient.userDiploma.deleteMany({
        where: {
          userId,
        },
      });
      for (const diploma of diplomas) {
        await this.createDiploma(userId, diploma.name, diploma.obtention);
      }
      return;
    } catch (error) {
      throw error;
