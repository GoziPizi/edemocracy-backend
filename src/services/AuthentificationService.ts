...
class AuthentificationService {
  ...
  static async registerFree(userInput: FreeUserCreateInputDto) {
    try {
      ...
      if (diplomas && user) {
        let finalDiplomas: { name: string; obtention: number }[] =
          JSON.parse(diplomas);
        for (let diploma of finalDiplomas) {
          await this.userRepository.createDiploma(
            user.id,
            diploma.name,
            diploma.obtention
          );
  ...
  static async registerFromPreRegistration(
    ...
    try {
      ...
      if (createdUser && sponsorshipCode) {
        ...
        let sponsor = await this.userRepository.getUserBySponsorshipCode(
          sponsorshipCode
        );
        if (sponsor) {
          await this.sponsorshipRepository.incrementJackpotOfUser(
            sponsor.id,
            1
          );
        }
      ...
      let diplomas =
        await this.preRegistrationRepository.getPreRegistrationDiplomas(
          preRegistration.id
        );
      ...
      if (diplomas) {
        ...
        await this.preRegistrationRepository.deletePreRegistrationDiplomas(
          preRegistration.id
        );
      }
