...
ContributionRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const session = await ContributionService.createStandardCheckoutSession(
        userId
      );
...
ContributionRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      await AuthentificationService.checkToken(token);
      const userId = AuthentificationService.getUserId(token);
      const session = await ContributionService.createPremiumCheckoutSession(
        userId
      );
      res.status(200).json({ url: session.url });
    } catch (error) {
...
ContributionRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const userId = AuthentificationService.getUserId(token);
      const session =
        await ContributionService.createBienfaiteurCheckoutSession(userId);
      res.status(200).json({ url: session.url });
    } catch (error) {
      next(error);
    }
