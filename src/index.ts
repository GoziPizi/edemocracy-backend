...
const sslFileDirectory =
  process.env.SSL_FILE_DIRECTORY ||
  '/etc/letsencrypt/live/digital-democracy.com/';
...
if (isProdEnvironment()) {
  try {
    privateKey = fs.readFileSync(
      path.join(sslFileDirectory, 'privkey.pem'),
      'utf8'
    );
    certificate = fs.readFileSync(
      path.join(sslFileDirectory, 'fullchain.pem'),
      'utf8'
    );
